...
from app.generation.models import PROMPT_VERSION, SCHEMA_VERSION, GenerationRequest
...
def main() -> int:
    ...
    parser.add_argument(
        "--offline-preflight",
        action="store_true",
        help="Validate/freeze manifest without opening retrieval or Gemini",
    )
    parser.add_argument("--execute", action="store_true", help="Run retrieval and build sample")
    parser.add_argument(
        "--allow-provider-call",
        action="store_true",
        help="Allow cache misses to call the production generation provider",
    )
                ...
                sample.append(
                    {
                        **item,
                        ...
                        "errorCode": str(exc) if str(exc).startswith("CACHE_MISS_") else type(exc).__name__,
                        "latencyMs": round((time.monotonic() - started) * 1000, 3),
                    }
                )
    finally:
    ...
    identity = {
        ...
        "corpusSha256": next(
            (item["corpusSha256"] for item in sample if item["status"] == "GENERATED"), None
        ),
        ...
        "generationTemperature": settings.gemini_generation_temperature,
        "generationCacheIdentities": [
            item["generationCacheIdentity"] for item in sample if item["status"] == "GENERATED"
        ],
    }
    report = {
        ...
        **manifest_report,
        ...
        "failedItems": len(sample) - generated,
        "failureCodes": dict(
            Counter(item.get("errorCode") for item in sample if item["status"] == "GENERATION_FAILED")
        ),
        "identity": identity,
        "teacherEvaluation": "NOT YET COLLECTED",
    }
    ...
    print(
        json.dumps(
            {"status": report["status"], "generated": generated, "failed": len(sample) - generated}, indent=2
        )
    )
