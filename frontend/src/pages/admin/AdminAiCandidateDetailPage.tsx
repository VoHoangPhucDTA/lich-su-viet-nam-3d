    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(20rem,1fr)]">
      <section className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
        ...
        <div className="flex flex-wrap gap-2 border-t border-[var(--border)] pt-4">
          ...
          {candidate.status === 'APPROVED' && canPublish && <><select aria-label="Đích xuất bản" className="admin-form-input" value={targetIndex} onChange={event => setTargetIndex(Number(event.target.value))}>{availableTargets.map((target, index) => <option key={target.sectionId} value={index}>{target.label}</option>)}</select><button className="admin-primary-button" disabled={busy || availableTargets.length === 0} onClick={() => setConfirmPublish(true)}>Xuất bản tường minh</button></>}
      ...
      <aside className="space-y-4">
        {isRevision && candidate.revision && <section className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
          ...
          <dl className="mt-3 grid gap-2 text-sm"><dt>Base official ID</dt><dd className="break-all font-mono text-xs">{candidate.revision.baseOfficialQuestionId}</dd><dt>Câu hỏi nền</dt><dd className="whitespace-pre-wrap">{candidate.revision.baseQuestionText}</dd><dt>Giải thích nền</dt><dd className="whitespace-pre-wrap">{candidate.revision.baseExplanation}</dd><dt>Độ khó / chủ đề</dt><dd>{candidate.revision.baseDifficulty} · {candidate.revision.baseTopic ?? '—'}</dd><dt>Content hash nền</dt><dd className="break-all font-mono text-xs">{candidate.revision.baseContentHash}</dd></dl>
