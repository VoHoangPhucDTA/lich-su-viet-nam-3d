...
> Goal 13D topology baseline: browser -> loopback Nginx frontend -> internal Spring -> internal FastAPI/MySQL. Goal 13E adds a guarded e2e-only deterministic provider; production defaults and runtime corpus/Chroma contracts are unchanged.
...
→ validators
→ source + output factual guard (`critical_facts_v1`)
→ at most one bounded repair
→ controlled success/failure
→ response with sourceChunkIds
```

RAG-02 does not replace retrieval. Chroma supplies the grounded source context; Gemini generates a structured candidate; the runtime guard then compares both source and output against 10 curated critical facts (`COUNT`, `DATE`, `PERSON`, `YEAR`). A source/registry conflict is fail-closed even when the generated value happens to be canonical.

Final thesis-level flow:

```text
React/Cesium → Spring Boot API → FastAPI AI service
                                  ├─ retrieval → Chroma
                                  ├─ generation → Gemini
                                  └─ bounded repair
                                       ↓
...
Event detail → Spring Boot TTS adapter → Viettel TTS (conditional, separate)
```
