...
## Final offline RAG-01/RAG-02 verification

Run from `ai-service/` without provider credentials or live-provider flags:

```powershell
python -m pytest tests/test_rag01.py -q
python -m pytest tests/test_rag02.py -q
python -m pytest -q -m "not integration"
python -m ruff check app scripts tests
python -m scripts.validate_critical_facts
```

Accepted final evidence is 33 RAG-01 tests PASS, 24 focused RAG-02 tests PASS, full offline suite 394 PASS/3 deselected/9 warnings, Ruff PASS, and zero provider calls. Do not rerun live generation merely to refresh documentation; use the immutable reports under `docs/review/fix-teacher/rag01-final/` and `rag02/`.
